function rampCurve(start, peak, end, length = 100) {
    const arr = [];
...
function flatCurve(value, length = 100) {
    return Array(length).fill(value);
}
/**
 * Attempt curve whose change accelerates toward the peak — models the
 * elbow bend in a lateral raise where torque grows fastest near horizontal.
 *
 * Uses a power-easing `t^exp` so the first half of the raise changes slowly
 * and the second half changes quickly.  `exp > 1` → late acceleration.
 */
function easedRampCurve(start, peak, end, exp = 2, length = 100) {
    const arr = [];
    const upLen = Math.floor(length / 2);
    for (let i = 0; i < length; i++) {
        if (i <= upLen) {
...
function constantBand(mean, stdVal, kind) {
 ...
 */
export function defaultLateralRaiseTemplate() {
    const armRaise = rampCurve(10, 90, 10);
    const symmetry = flatCurve(0);
    const torsoLean = flatCurve(0); // ideal: upright (0°); std 10 gives wide band
    const hipDrift = flatCurve(0);
    const kneeValgus = flatCurve(0);
    const elbow = easedRampCurve(160, 140, 160, 2); // non-linear: slow start, accelerates near peak
